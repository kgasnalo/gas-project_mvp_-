 ...
 *
 * データパターン:
 * - C001: 理想的なケース（志望度上昇、接点密、高評価） ⚠️ Candidates_Masterに存在する場合のみ
 * - C002: 志望度下降ケース（興味減少、懸念増加）
 * - C003: 高評価ケース（全スコア高い）
 * - C004: 低評価ケース（接点少ない、志望度低い）
 * - C005/C010: 標準ケース（平均的なスコア） ⚠️ C005またはC010として登録
 *
 * ⚠️ 重要な注意事項:
 * - このスクリプトは既存のCandidates_Masterにデータを追加します
 * - C001, C002, C003, C004がCandidates_Masterに存在している必要があります
 * - C005またはC010（ito@example.com）が存在している必要があります
 * - 存在しない候補者のデータは自動チェック時にスキップされます
 */
...
function insertSurvey初回面談() {
  ...
  const responses = [
    // ⚠️ 注意: C001（tanaka@example.com）がCandidates_Masterに存在しない場合、
    // このデータは自動チェック時にスキップされます。
    // C001を使用する場合は、事前にCandidates_Masterに登録してください。
