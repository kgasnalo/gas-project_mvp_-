...
| B6 | 平均承諾可能性 | `=ROUND(AVERAGE(Candidates_Master!R:R),1) & "点"` | `=ROUND(AVERAGE(Candidates_Master!H:H),1) & "点"` |
...
📋 確認事項:
1. Dashboardシートを開く
2. B6: 平均承諾可能性が正常な値（74.3点など）
3. B7: 高確率候補者数が正常な値（3名など）
4. B8: 要注意候補者数が正常な値（3名など）
...
【KPIサマリー】
総候補者数                          10名
平均承諾可能性                      74.3点  ← ✅ エラーなし、正常な値
高確率候補者数（80点以上）           3名     ← ✅ 正常な件数
要注意候補者数（60点未満）           3名     ← ✅ 正常な件数
本日の新規記録                      0件
人間の直感入力率                    60%
```
...
- [ ] Dashboard B6に「74.3点」のような正常な値が表示されている
- [ ] Dashboard B7に「3名」のような正常な件数が表示されている
- [ ] Dashboard B8に「3名」のような正常な件数が表示されている
