...
/**
 * Contact_Historyにダミーデータを投入（Phase 3-2用）
 *
 * C001〜C005の候補者について、接点回数を変えてスコアのバリエーションを出す
 * - C001: 8件（接点多い → 高スコア期待）
 * - C002: 5件（接点普通 → 中スコア期待）
 * - C003: 10件（接点非常に多い → 最高スコア期待）
 * - C004: 3件（接点少ない → 低スコア期待）
 * - C005: 6件（接点やや多い → 中〜高スコア期待）
 */
function insertContactHistoryData() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    // C001: 8件（密な接点、最近も活発）
    testData.push(
      ['CONT-C001-001', 'C001', '田中太郎', new Date(2025, 8, 15, 14, 0), 'メール', '田中リクルーター', '初回コンタクト。職務経歴書の確認', new Date(2025, 8, 18)],
      ['CONT-C001-002', 'C001', '田中太郎', new Date(2025, 8, 22, 15, 30), 'カジュアル面談', '佐藤課長', 'カジュアル面談。会社説明と質疑応答', new Date(2025, 8, 29)],
      ['CONT-C001-003', 'C001', '田中太郎', new Date(2025, 9, 1, 10, 0), '面接', '山田部長', '1次面接。技術面接', new Date(2025, 9, 8)],
      ['CONT-C001-004', 'C001', '田中太郎', new Date(2025, 9, 10, 11, 0), '電話', '田中リクルーター', '選考状況フォロー', new Date(2025, 9, 17)],
      ['CONT-C001-005', 'C001', '田中太郎', new Date(2025, 9, 18, 14, 0), '面談', '鈴木係長', '社員面談。現場メンバーとの交流', new Date(2025, 9, 25)],
      ['CONT-C001-006', 'C001', '田中太郎', new Date(2025, 9, 26, 15, 0), '面接', '山田部長', '2次面接。マネージャー面接', new Date(2025, 10, 3)],
      ['CONT-C001-007', 'C001', '田中太郎', new Date(2025, 10, 5, 16, 0), '面接', '人事部', '最終面接。役員面接', new Date(2025, 10, 12)],
      ['CONT-C001-008', 'C001', '田中太郎', new Date(2025, 10, 10, 10, 0), 'オファー面談', '山田部長', '内定条件の提示と質疑応答', null]
    );

    // C002: 5件（標準的な接点）
    testData.push(
      ['CONT-C002-001', 'C002', '佐藤花子', new Date(2025, 8, 20, 14, 0), 'メール', '田中リクルーター', '初回コンタクト', new Date(2025, 9, 3)],
      ['CONT-C002-002', 'C002', '佐藤花子', new Date(2025, 9, 5, 15, 0), 'カジュアル面談', '佐藤課長', 'カジュアル面談', new Date(2025, 9, 19)],
      ['CONT-C002-003', 'C002', '佐藤花子', new Date(2025, 9, 20, 10, 0), '面接', '山田部長', '1次面接', new Date(2025, 10, 4)],
      ['CONT-C002-004', 'C002', '佐藤花子', new Date(2025, 10, 6, 14, 0), '面接', '人事部', '2次面接', new Date(2025, 10, 20)],
      ['CONT-C002-005', 'C002', '佐藤花子', new Date(2025, 10, 5, 15, 0), '電話', '田中リクルーター', '選考フォロー', null]
    );

    // C003: 10件（非常に密な接点）
    testData.push(
      ['CONT-C003-001', 'C003', '鈴木一郎', new Date(2025, 8, 18, 14, 0), 'メール', '田中リクルーター', '初回コンタクト', new Date(2025, 8, 23)],
      ['CONT-C003-002', 'C003', '鈴木一郎', new Date(2025, 8, 24, 15, 0), 'カジュアル面談', '佐藤課長', 'カジュアル面談', new Date(2025, 8, 30)],
      ['CONT-C003-003', 'C003', '鈴木一郎', new Date(2025, 9, 1, 10, 0), '面接', '山田部長', '1次面接', new Date(2025, 9, 7)],
      ['CONT-C003-004', 'C003', '鈴木一郎', new Date(2025, 9, 8, 11, 0), '電話', '田中リクルーター', '選考フォロー', new Date(2025, 9, 14)],
      ['CONT-C003-005', 'C003', '鈴木一郎', new Date(2025, 9, 15, 14, 0), '面談', '鈴木係長', '社員面談1回目', new Date(2025, 9, 21)],
      ['CONT-C003-006', 'C003', '鈴木一郎', new Date(2025, 9, 22, 15, 0), '面談', '鈴木係長', '社員面談2回目', new Date(2025, 9, 28)],
      ['CONT-C003-007', 'C003', '鈴木一郎', new Date(2025, 9, 29, 16, 0), '面接', '山田部長', '2次面接', new Date(2025, 10, 5)],
      ['CONT-C003-008', 'C003', '鈴木一郎', new Date(2025, 10, 6, 10, 0), 'メール', '田中リクルーター', '追加資料送付', new Date(2025, 10, 12)],
      ['CONT-C003-009', 'C003', '鈴木一郎', new Date(2025, 10, 13, 15, 0), '面接', '人事部', '最終面接', new Date(2025, 10, 19)],
      ['CONT-C003-010', 'C003', '鈴木一郎', new Date(2025, 10, 12, 10, 0), 'オファー面談', '山田部長', '内定条件提示', null]
    );

    // C004: 3件（接点少ない）
    testData.push(
      ['CONT-C004-001', 'C004', '高橋美咲', new Date(2025, 8, 25, 14, 0), 'メール', '田中リクルーター', '初回コンタクト', new Date(2025, 9, 15)],
      ['CONT-C004-002', 'C004', '高橋美咲', new Date(2025, 9, 18, 15, 0), 'カジュアル面談', '佐藤課長', 'カジュアル面談', new Date(2025, 10, 9)],
      ['CONT-C004-003', 'C004', '高橋美咲', new Date(2025, 9, 28, 10, 0), '面接', '山田部長', '1次面接', null]
    );

    // C005: 6件（やや密な接点）
    testData.push(
      ['CONT-C005-001', 'C005', '渡辺健太', new Date(2025, 8, 22, 14, 0), 'メール', '田中リクルーター', '初回コンタクト', new Date(2025, 9, 2)],
      ['CONT-C005-002', 'C005', '渡辺健太', new Date(2025, 9, 4, 15, 0), 'カジュアル面談', '佐藤課長', 'カジュアル面談', new Date(2025, 9, 14)],
      ['CONT-C005-003', 'C005', '渡辺健太', new Date(2025, 9, 16, 10, 0), '面接', '山田部長', '1次面接', new Date(2025, 9, 26)],
      ['CONT-C005-004', 'C005', '渡辺健太', new Date(2025, 9, 28, 11, 0), '電話', '田中リクルーター', '選考フォロー', new Date(2025, 10, 8)],
      ['CONT-C005-005', 'C005', '渡辺健太', new Date(2025, 10, 10, 14, 0), '面接', '人事部', '2次面接', new Date(2025, 10, 20)],
      ['CONT-C005-006', 'C005', '渡辺健太', new Date(2025, 10, 8, 15, 0), 'メール', '田中リクルーター', '追加情報送付', null]
    );

    // データを書き込む
    if (testData.length > 0) {
      sheet.getRange(2, 1, testData.length, 8).setValues(testData);
      Logger.log(`✅ Contact_Historyに${testData.length}件のダミーデータを投入しました`);
    }

  } catch (error) {
    Logger.log(`❌ Contact_Historyダミーデータ投入エラー: ${error.message}`);
    Logger.log(error.stack);
  }
}
